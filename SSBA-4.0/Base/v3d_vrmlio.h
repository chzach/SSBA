...
namespace V3D
{
   ...
   template <typename U>
   void writeCameraFrustumToVRML(CameraMatrix const& cam, double const width, double const height, double const scale,
                                 U const& color255, const char *filename, bool append)
   {
      ...
      Vector2d const pp     = cam.getPrincipalPoint();
      Vector3d const center = cam.cameraCenter();
      double const w2 = width/2, h2 = height/2;

      Vector3d const ll = cam.unprojectPixel(makeVector2(-w2 + pp[0],  h2 + pp[1]), scale);
      Vector3d const lr = cam.unprojectPixel(makeVector2( w2 + pp[0],  h2 + pp[1]), scale);
      Vector3d const ul = cam.unprojectPixel(makeVector2(-w2 + pp[0], -h2 + pp[1]), scale);
      Vector3d const ur = cam.unprojectPixel(makeVector2( w2 + pp[0], -h2 + pp[1]), scale);
